...
const CONTROLLERS = {
  ...
  dashboard:'http://192.168.0.60', // Dashboard controller
  room:    'http://192.168.0.80',  // Room controller (testroom / R-EETPL)
  room75:  'http://192.168.0.75',  // R-BandB
  room76:  'http://192.168.0.76',  // R-BADK
  room77:  'http://192.168.0.77',  // R-INKOM
  room78:  'http://192.168.0.78',  // R-KEUKEN
  room79:  'http://192.168.0.79',  // R-WASPL
  room80:  'http://192.168.0.80',  // R-EETPL
  room81:  'http://192.168.0.81',  // R-ZITPL
};
...
// ── Photon proxy via Cloudflare Worker ───────────────────────────────────────
// Voorkomt CORS-problemen en TLS in de browser
...
app.get('/api/photon/:id', async (req, res) => {
  try {
    ...
    res.status(503).json({ online: false, error: 'Photon offline', detail: e.message });
  }
});

// ── Matrix data — alle controllers in één call ────────────────────────────────
// Haalt ESP32 + Photon data op voor de matrix.html pagina
// ESP32s worden parallel gepolld, Photons via worker proxy
const PHOTON_IDS = {
...
app.get('/api/matrix', async (req, res) => {
  ...
  // Alle ESP32 controllers parallel ophalen
  ...
  // Photons parallel ophalen via Cloudflare Worker
