...
**RPi = display + controle + energie-analyse laag**
...
- Neemt GEEN dataverzameling of logging taken over van Dashboard
- Pollt S-ENERGY `/json` voor energiedata
- Berekent dagcumulatieven en maandstatistieken energie per huis
- Slaat energiestatistieken op in `stats-energie.json`
- Berekent en bewaart maandpiek per huis (SCH, WON, gecombineerd)
- Haalt EPEX data op, berekent all-in prijs → serveert als `/api/tariff`
- Berekent koken/wassen advies op basis van EPEX + solar
- Leest P1-dongle data voor WON (~2028)
- Genereert maandelijks afrekeningsrapport → `/afrekening`
...
| 11 | Smart Energy controller — sketch v1.24 ✅ — commissioning ⬜ | ⬜ Open |
...
| AP9 | S-ENERGY: interface printje bouwen (UTP → Roomsense RJ45, pull-up + serieweerstanden per S0-kanaal) |
| AP10 | S-ENERGY: sketch v1.24 testen + commissioning |
| AP11 | S-ENERGY: RPi /api/tariff endpoint bouwen (EPEX all-in + adviezen) |
...
## 11. Smart Energy Controller (S-ENERGY)

> **Status: in ontwikkeling — sketch v1.24 beschikbaar.**
> IP: `192.168.0.73` · Board: ESP32-C6 32-pin · Filename: `ESP32_C6_ENERGY_v1_24.ino`

### 11.0 Taakverdeling ESP32 S-ENERGY

**ESP32 doet (lokale I/O + doorgeefluik):**
- S0 pulsen tellen via ISR (3 kanalen: Solar fwd, SCH fwd, SCH rev)
- Real-time vermogen W berekenen via pulsinterval
- Ruwe Wh-tellers bijhouden in RAM
- `/json` serveren met ruwe meetwaarden → gepollt door Dashboard (→ Google Sheets) én RPi
- `/api/tariff` ophalen van RPi (EPEX all-in prijs + adviezen) → opslaan als lokale cache
- Matrix 12×4 updaten vanuit eigen S0-data + RPi tariefcache
- Bij RPi-uitval: matrix blijft werken met eigen S0-data + laatste bekende tariefcache
- P1-dongle data ophalen via WiFi voor WON (~2028)
- Standaard Zarlar infrastructuur (WiFi, OTA, /settings, /factory_reset, AP captive portal)

**RPi doet (berekening + opslag + presentatie):**
Zie §2.1
...
|---|---|---|---|---|---|
| S0-1 | Zonnepanelen (A14) | 18/19 | Forward | IO5 | Solar productie |
| S0-2 | Schuur (A5) | 18/19 | Forward | IO6 | SCH afname van net |
| S0-3 | Schuur (A5) | 20/21 | Reverse | IO7 | SCH injectie naar net (incl. batterij later) |
...
Verbinding via **Roomsense RJ45** connector op het Zarlar shield.
